...
class StockDataset(Dataset):
    ...
    def __init__(
        self, path: str, tags: Literal["train", "val", "test"], train_ratio: float = 0.8
    ) -> None:
        super().__init__()
        self.path = path
        self.tags = tags
    ...
    def __readdata__(self) -> Tuple[np.ndarray | Any | list, np.ndarray | Any | list]:
        """
        Read data
        """
        ...
        if self.tags == "test":
            ...
            raise ValueError(f"Invalid tag: {self.tags}")
...
def build_dataloader(
    ...
    Args:
        tags (Literal["train", "val", "test"]): tags
        data_conf (Dict[str, Any]): data configuration
    """
    input_dim, output_dim = 0, 0

    dataset = StockDataset(
        path=data_conf["path"], tags=tags, train_ratio=data_conf["train_ratio"]
    )
